...
import mythos.energy.dna2 as jd_energy2
import mythos.energy.rna2 as jd_energy
...
def get_setup_data(base_dir: str):
    ...
    default_params = jd_toml.parse_toml("mythos/input/rna2/default_energy.toml")
...
def test_bonded_excluded_volume(base_dir: str):
    ...
    energy_fn = jd_energy1.BondedExcludedVolume(
        displacement_fn=displacement_fn,
        transform_fn=transform_fn,
        topology=topology,
        params=energy_config.init_params(),
...
@pytest.mark.parametrize(
    ("base_dir", "t_kelvin", "weights", "use_pseq"),
    [
...
def test_stacking(base_dir: str, t_kelvin: float, weights: jnp.ndarray, *, use_pseq: bool):
    ...
    energy_config = jd_energy.StackingConfiguration(
        **(default_params["stacking"] | {"kt": t_kelvin * 0.1 / 300.0}),
...
def test_unbonded_excluded_volume(base_dir: str):
    ...
    energy_fn = jd_energy1.UnbondedExcludedVolume(
        displacement_fn=displacement_fn,
        transform_fn=transform_fn,
        topology=topology,
        params=energy_config.init_params(),
...
def test_hydrogen_bonding(base_dir: str):
    ...
    energy_fn = jd_energy1.HydrogenBonding(
        displacement_fn=displacement_fn,
        transform_fn=transform_fn,
        topology=topology,
        params=energy_config.init_params(),
...
def test_coaxial_stacking(base_dir: str):
    ...
    energy_fn = jd_energy1.CoaxialStacking(
        displacement_fn=displacement_fn,
        transform_fn=transform_fn,
        topology=topology,
        params=energy_config.init_params(),
