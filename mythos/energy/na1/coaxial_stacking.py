...
class CoaxialStacking(je_base.BaseEnergyFunction):
    ...
    def compute_energy(self, nucleotide: na1_nucleotide.HybridNucleotide) -> typ.Scalar:
        ...
        dna_dgs = dna2_energy.CoaxialStacking.create_from(self, params=self.params.dna_config).pairwise_energies(
            nucleotide.dna,
            nucleotide.dna,
            self.unbonded_neighbors,
        )

        rna_dgs = dna1_energy.CoaxialStacking.create_from(self, params=self.params.rna_config).pairwise_energies(
            nucleotide.rna,
            nucleotide.rna,
            self.unbonded_neighbors,
        )

        drh_dgs = dna1_energy.CoaxialStacking.create_from(self, params=self.params.drh_config).pairwise_energies(
            nucleotide.dna,
            nucleotide.rna,
            self.unbonded_neighbors,
        )

        rdh_dgs = dna1_energy.CoaxialStacking.create_from(self, params=self.params.drh_config).pairwise_energies(
            nucleotide.rna,
            nucleotide.dna,
            self.unbonded_neighbors,
        )
