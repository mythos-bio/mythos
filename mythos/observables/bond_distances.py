...
class BondDistances:
    """Observable for computing bond distances.
    ...
    def _matching_pairs(self, bond_name: str) -> jnp.ndarray:
        ...
        indices = [i for i, name in enumerate(all_names) if name == bond_name]
        if not indices:
            raise ValueError(
                f"No bonds matching '{bond_name}' found in the topology. "
    ...
    def __call__(self, trajectory: SimulatorTrajectory) -> dict[str, jnp.ndarray]:
        ...
        for bond_name in self.bond_names:
            pairs = self._matching_pairs(bond_name)
            ...
            distances = jax.vmap(_distances_for_state)(trajectory.center, trajectory.box_size)
